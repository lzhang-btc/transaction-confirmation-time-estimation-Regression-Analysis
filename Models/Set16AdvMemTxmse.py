...
dir_abbv='finalAttentionssigmoidAdvMemTx'
...
def selfAttentionmempoolModel(layers,lstmunits,lstmtimestamps,blockFeatureDim,MemFeatureDim,txFeatureDim):
  np.random.seed(9)
  ...
  model2_merged_vector = keras.layers.concatenate([self_attention_out2,model2_auxiliary_input], axis=-1)
...
fel_model = selfAttentionmempoolModel(layers, lstmunits, lstmtimestamps, BocFeaLens,MemFeaLens, TxFeaLens)
plot_model(fel_model, to_file=dir_abbv+'.png', show_shapes=True)


filepath_fel = './TrainedModels/Set'+str(TestGroup)+str(currentHeight)+'Adv.h5'
...
else: #####When the Testing dataset is empty, all the three indicators are zero
