...
dir_abbv='finalAttentionssigmoidQVK'
...
def QVKSelfAttentionmempoolModel(layers,lstmunits,lstmtimestamps,blockFeatureDim,MemFeatureDim,txFeatureDim):
  np.random.seed(9)
...
fel_model = QVKSelfAttentionmempoolModel(layers, lstmunits, lstmtimestamps, BocFeaLens,MemFeaLens, TxFeaLens)
plot_model(fel_model, to_file=dir_abbv+'.png', show_shapes=True)


filepath_fel = './TrainedModels/Set'+str(TestGroup)+str(currentHeight)+'QVK.h5'
