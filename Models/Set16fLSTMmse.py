...
dir_abbv='finalFixedBlockMemIntervalLSTMsigmoid'
...
blocksfortrain = blockcsv[(blockcsv[blockHeightBinx]>=currentHeight-training_blocks)&(blockcsv[blockHeightBinx]<currentHeight+total_EstimateBlock)]
...
blockHeightMinx=0
memsfortrain = memcsv[(memcsv[blockHeightMinx]>=currentHeight-training_blocks)&(memcsv[blockHeightMinx]<currentHeight+total_EstimateBlock)]
...
def BlockMemLSTMModel(layers,lstmunits,lstmtimestamps,blockFeatureDim,MemFeatureDim,txFeatureDim):
  np.random.seed(9)
  ...
  model2_lstm_out2 = LSTM(lstmunits)(model2_input2)

  model2_auxiliary_input = Input(shape=(txFeatureDim,), name='model2_tx_input')

  model2_merged_vector = keras.layers.concatenate([model2_lstm_out, model2_lstm_out2, model2_auxiliary_input], axis=-1)
...
def constructMemSeries(curHeight, txdata,weightVectorLen,lstmtimestamps):
    txcollection=np.array(txdata)
    seriesList=[]
    for i in range(lstmtimestamps):
        obs_height=curHeight-lstmtimestamps+1+i
        mempool_state = [0 for i in range (weightVectorLen)]

        txs_selected = txcollection[np.where((txcollection[:, enterBlockintx] <= curHeight) &
                                             (txcollection[:, blockHeightintx] >= curHeight))]
        for tx in txs_selected:
            feerate = int(tx[feerateintx])
            weight=tx[weightintx]
            if feerate>weightVectorLen-1:
                feerate=weightVectorLen-1
            mempool_state[feerate] = mempool_state[feerate] + weight
        seriesList.append(mempool_state)
...
train_txfeatureList1,train_txOutputList1,strain_blockSeqList1,strain_memSeqList1=txDatasetConstruction(txfile,blockfile,memfile,lstmtimestamps,currentHeight-training_blocks,currentHeight+total_EstimateBlock)
#######Scale Features
...
fel_model = BlockMemLSTMModel(layers, lstmunits, lstmtimestamps, BocFeaLens,MemFeaLens, TxFeaLens)
plot_model(fel_model, to_file=dir_abbv+'.png', show_shapes=True)


filepath_fel = './TrainedModels/Set'+str(TestGroup)+str(currentHeight)+'LSTM.h5'
